...
public protocol GraniteIPFSBase {
...
protocol ClientSubCommand {
    var parent: GraniteIPFSBase? { get set }
}

extension ClientSubCommand {
    mutating func setParent(_ p: GraniteIPFSBase) { self.parent = p }
}

extension GraniteIPFSBase {
...
//	stream.close()
    ...
    func fetchJson(_ path: String, completionHandler: @escaping (JsonType) throws -> Void) throws {
        try fetchData(path) {
            ...
            GraniteLogger.info("The data: \(NSString(data: data, encoding: String.Encoding.utf8.rawValue) as! String)")
            let fixedData: Data = fixStreamJson(data)
            GraniteLogger.info("The fixed data: \(NSString(data: fixedData, encoding: String.Encoding.utf8.rawValue) as! String)")
...
public class GraniteIPFS: GraniteIPFSBase {
