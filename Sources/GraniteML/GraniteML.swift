...
public class GraniteML<T> {
    ...
    public init(
        ...
        modelUrl = bundle.url(forResource: "\(model.self)", withExtension: "mlmodelc")
...
extension GraniteML {
    ...
    public func predictForImageInput(
        ...
        case .image:
            guard let modelOutput = try? pipeline.modelIO?.run(
                    on: CoreMLInput.image(input as! GraniteImage)) else {
                return nil
            }

            let output = modelOutput.values.compactMap({ $0 as? Output }).first
            if output == nil,
               let outputMulti = modelOutput.values.compactMap({ $0 as? MLMultiArray }).first {
                return outputMulti.image() as? Output
            } else {
                return output
            }
        default:
